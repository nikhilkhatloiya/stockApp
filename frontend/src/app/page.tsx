...
{/* Main Content */}
<main className="flex-1 flex flex-col items-center justify-start px-4 py-8 relative z-10">
  <div className="w-full max-w-7xl">
    {/* All your stock dashboard content */}
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Connection Status */}
      <div className="mb-6 flex justify-center">
        <div className={`flex items-center px-4 py-2 rounded-full text-sm font-medium ${isConnected
          ? 'bg-emerald-50 text-emerald-600 border border-emerald-200'
          : 'bg-red-50 text-red-600 border border-red-200'
          }`}>
          <div className={`w-2 h-2 rounded-full mr-2 ${isConnected ? 'bg-emerald-600 animate-pulse' : 'bg-red-600'}`}></div>
      ...
      <div className="mb-8 space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 transition"
          />
        </div>
      ...
      {/* Loading / Disconnected / Stock Grid */}
      {stocks.length === 0 && (
        <div className="text-center py-16">
          {isConnected ? (
            <>
              <div className="animate-spin rounded-full h-16 w-16 border-4 border-orange-500 border-t-transparent mx-auto mb-4"></div>
              <p className="text-gray-900 text-lg">Loading stock data...</p>
            </>
          ) : (
            <>
              <div className="text-8xl mb-6">🔌</div>
              <h3 className="text-2xl font-semibold text-gray-900 mb-4">Connecting to server...</h3>
      ...
      {filteredStocks.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          {filteredStocks.map((stock, index) => (
            <div
              key={stock.symbol}
              className="group bg-white rounded-2xl p-6 hover:shadow-2xl hover:scale-105 transition-all duration-300"
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className="flex-1">
                  ...
                  {stock.metadata?.exchange && (
                    <span className="inline-block bg-orange-50 text-orange-600 text-xs px-2 py-1 rounded-full mt-2">
                      {stock.metadata.exchange}
                    </span>
                  )}
                </div>
              ...
              <div className="space-y-3 text-sm mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-gray-900">{formatCurrency(stock.previousClose)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-500">Day Range</span>
                  <span className="font-medium text-gray-900">{formatCurrency(stock.dayLow)} - {formatCurrency(stock.dayHigh)}</span>
                </div>
                <div className="flex justify-between items-center">
              ...
              <Link
                ...
                className="w-full bg-gradient-to-r from-orange-500 to-pink-500 hover:opacity-90 text-white font-medium py-3 px-4 rounded-xl transition-all duration-200 inline-block text-center"
              >
                View Details →
              </Link>
            </div>
      ...
      {stocks.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-emerald-600 mb-2">{stocks.filter(s => s.change > 0).length}</div>
              <div className="text-sm text-gray-500">Gainers</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-red-600 mb-2">{stocks.filter(s => s.change < 0).length}</div>
              <div className="text-sm text-gray-500">Losers</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-gray-500 mb-2">{stocks.filter(s => s.change === 0).length}</div>
      ...
      <div className="flex flex-wrap gap-4 justify-center">
        <Link
          href="/portfolio"
          className="bg-gradient-to-r from-orange-500 to-pink-500 hover:opacity-90 text-white font-medium px-8 py-4 rounded-xl transition-all duration-200 flex items-center space-x-3 shadow-md"
        >
        ...
        <Link
          href="/api-docs"
          className="bg-gradient-to-r from-gray-500 to-gray-600 hover:opacity-90 text-white font-medium px-8 py-4 rounded-xl transition-all duration-200 flex items-center space-x-3 shadow-md"
        >
          <span className="text-xl">📋</span>
